package 
{
	...
	public class xSheet extends Screen
	{
		...
		private static var startRecord:int = 0;
		
		public function xSheet()
		{
		}
...
//			}
			...
			animation = new ListCollection(
				[//{ text: "a", thumbnail: Assets.getTexture("Ring")}// atlas.getTexture( "run0001" ) },
...
//					{ text: "d", thumbnail: atlas.getTexture( "run0004" ) },
		...
		/** clik shift 1 on to record and shift 1 to finish to print out the values hold shift on the beginning frame you want to record
		 * then the one you want to end on  best part its already set up in flash too when your done**/
		public static function ReturnClicked():void
		{
			...
			//	Main.liveCamera.hero_mc.inAlter = true
				// hero frames was just a shortcut to get to what I knew was hero but the others are accessable just as easy with the for loop
				for (i = 0; i < FrameHandler.heroFrames.length; i++) 
				{
					
					// this will fail if you dong have xheet up
					animation.push({ text: (Number(i+1)).toString() })
				}
				if(!actor){
					actor = Main.liveCamera.hero_mc
				}
				//actor.isFrozen = true;
				...
				Main.liveCamera.hero_mc.isFrozen = false
				var actr:Object = frames[animation.length-1];
//							actor.body.position.x = actr.bodyx
//							actor.body.position.y = actr.bodyy
				MainSceneMotor.mouseJoint.anchor1.setxy(actr.bodyx,actr.bodyy);
				EditKeyHandler.State = EditKeyHandler.Animation;
				// may want this not sure
				//EditKeyHandler.State = EditKeyHandler.Animation;
			}
			// ok i dont know what this does but i dont care soo im just gonna go back in here and make my own new one thats easy
		...
		private function list_changeHandler( event:Event ):void
		{
			...
			selectFrame();
			}else if(false && EditKeyHandler.State == EditKeyHandler.EditAnimation){
				editFrame();
			}
		}
		...
		public static function selectFrame():void
		{
			...
			// this is if i select a frame that i altered that was edited how do i get it on screen this was my solution... you know this all figures out a lot of things i may need if i do this for real, suprisingly this may be usefull
//			if(frameMarked){
//			writeFrameToMemory(prevActr, prevIndex);
//			if(keyFrames[prevActr.anim] == undefined)
//			{
...
//			actor.body.position.y = actr.bodyy
			...
			if(EditKeyHandler.shift){
				...
				list.allowMultipleSelection = true
					// we'll do it one way first then we'll do it the other
					if(prevIndex <= list.selectedIndex){
						var start:int = prevIndex
						var fin:int = list.selectedIndex
					}else{
						start = list.selectedIndex
						fin = prevIndex
					}
					var vals:String = ""
					for (var i:int = start; i <= fin; i++) 
					{
						if(i != start)
						vals += ","
						list.selectedIndices.push(i)
						vals+= "{X:"+frames[i].bodyx+",Y:"+frames[i].bodyy+",Z:0}"
					}
					trace("["+vals+"]")
				...
				// it seems a bit insane to keep saved animations
...
//			var p:Point = MainSceneMotor.local3DToGlobal(new Vector3D(actor.X,actor.Y,actor.Z))
//			p = Main.liveCamera.localToGlobal(p)
//				// oh jeez this creates a bitmap so i can edit it on space, wow the amount of work i did for this was above and beyond... stupid but admirable
//			LevelEditor.bm.x = p.x;
//			LevelEditor.bm.y = p.y;
